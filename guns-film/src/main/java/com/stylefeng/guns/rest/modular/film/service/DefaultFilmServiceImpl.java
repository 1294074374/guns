...
// 如果sourceId,yearId,catId 不为99 ,则表示要按照对应的编号进行查询
    ...
    public List<FilmInfo> getTop() {
        ...
        entityWrapper.eq("film_status", "1");
        //查询条件
        Page<MoocFilmT> page = new Page<>(1, 10, "film_source");
        List<MoocFilmT> moocFilms = moocFilmTMapper.selectPage(page, entityWrapper);
        List<FilmInfo> filminfos = getFilmInfos(moocFilms);
        return filminfos;
    }
    ...
    public List<SourceVO> getSources() {
        ...
        //转换成业务对象
        for (MoocSourceDictT moocSourceDict : moocSourceDictTs) {
            SourceVO source = new SourceVO();
            source.setSourceId(moocSourceDict.getUuid() + "");
            source.setSourceName(moocSourceDict.getShowName());
    ...
    public List<YearVO> getYears() {
        ...
        //转换成业务对象
        for (MoocYearDictT moocYearDict : moocYearDicts) {
            YearVO year = new YearVO();
            year.setYearId(moocYearDict.getUuid() + "");
            year.setYearName(moocYearDict.getShowName());
