...
public class DefaultAlipayServiceImpl implements AliPayServiceAPI {
    ...
    public AlipayInfoVO getQRCode(String orderId) {
        String filmPath = trade_precreate(orderId);
        if (filmPath == null || filmPath.trim().length() == 0) {
            return null;
        } else {
            AlipayInfoVO alipayInfoVO = new AlipayInfoVO();
            alipayInfoVO.setOrderId(orderId);
            alipayInfoVO.setQRCodeAddress(filmPath);
            return alipayInfoVO;
        }
    }
    ...
    public String trade_precreate(String orderId) {
        ...
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，mooccaesar123123
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("E:/qrcode/qr-%s.png", response.getOutTradeNo());
                String fileName = String.format("qr-%s.png", response.getOutTradeNo());
                log.info("filePath:" + filePath);
                File rqCodeImge = ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                boolean isSuccess = ftpUtil.uploadFile(fileName, rqCodeImge);
                if (!isSuccess) {
                    filePath = "";
                    log.error("二维码上传服务器失败");
                }
                break;
    ...
    public boolean trade_query(String orderId) {
        ...
        String outTradeNo = "mooccaesar123123";
